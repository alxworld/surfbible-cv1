    ...
    <main className="bg-green-50 min-h-[calc(100svh-3.5rem)]">
      <div className="max-w-lg mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <Link href="/admin" className="text-sm text-emerald-600 hover:text-emerald-700 font-medium">← Admin</Link>
            <h1 className="text-2xl font-bold text-stone-900 mt-1">{church.name} — Members</h1>
          </div>
        </div>

        <div className="bg-white border border-emerald-100 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-emerald-50">
            <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide">{members.length} member{members.length !== 1 ? "s" : ""}</p>
          </div>
          {rows.length === 0 ? (
            <p className="text-sm text-stone-400 px-5 py-4">No members yet.</p>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-stone-900 truncate">
                        ...
                        {m.id === user.id && <span className="ml-1.5 text-[10px] text-stone-400">(you)</span>}
                        {church.createdBy === m.id && <span className="ml-1.5 text-[10px] text-emerald-600 font-semibold uppercase">admin</span>}
                      </p>
                  ...
                  {m.enrollments.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {m.enrollments.map((e, i) => (
                        <span key={i} className="text-xs bg-emerald-50 border border-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">
                          Day {e.currentDay} · {e.streakCount}🔥
                        </span>
                      ))}
                    </div>
                  )}
